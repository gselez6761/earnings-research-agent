...
def main(ticker: str, thread_id: str) -> None:
    ...
    if state.next and state.next[0] == "human_review_node":
        ...
        # In production, a frontend polls this state, renders the UI, and the 
        # analyst submits a payload. We mock this via CLI input.
        print("\n" + "="*40)
        ...
        feedback = HumanFeedback(
            ...
            quality_rating=4  # Arbitrary mock rating
        )

        logger.info("Submitting human feedback: %s", action.value)

        # Resume the graph by passing the feedback object into Command(resume=...)
        for event in graph.stream(Command(resume=feedback), config=config, stream_mode="values"):
            pass  # Let the graph route to refine_node or finish
...
if __name__ == "__main__":
    ...
    main(args.ticker, args.thread_id)